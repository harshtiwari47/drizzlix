  ...
  return (
    ...
    >
      <div className="draft-header" style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '1.5rem' }}>
        <div>
          <h2 style={{ fontFamily: 'var(--font-display)', color: 'var(--text-primary)', margin: 0, fontSize: '1.8rem', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
      ...
      <div className="draft-meta-forms" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        ...
        {String(draftDeck.generationPrompt || '').trim() && (
          <div style={{ gridColumn: '1 / -1', background: 'rgba(42, 58, 88, 0.22)', border: '1px solid rgba(151, 194, 255, 0.26)', borderRadius: 'var(--radius-lg)', padding: '0.95rem' }}>
            <p style={{ margin: 0, color: 'var(--text-primary)', fontSize: '0.84rem', fontWeight: 700, fontFamily: 'var(--font-body)' }}>
              Original AI Prompt
            </p>
      ...
      <div className="draft-cards-list" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' }}>
        <h3 style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-display)', margin: 0, fontSize: '1.2rem', paddingBottom: '0.5rem', borderBottom: '1px solid var(--card-border)' }}>Synthesized Nodes ({draftDeck.cards.length})</h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxHeight: '500px', overflowY: 'auto', paddingRight: '0.5rem' }}>
          {draftDeck.cards.map((card, idx) => (
            <div key={card.id} style={{ display: 'flex', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 'var(--radius-lg)', padding: '1.2rem', alignItems: 'flex-start', gap: '1.2rem', transition: 'all 0.2s ease' }} onMouseOver={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)' }} onMouseOut={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.05)' }}>
              <div style={{ background: 'linear-gradient(135deg, #1f1f1f, #2a2a2a)', color: 'var(--primary)', border: '1px solid var(--card-border)', width: '32px', height: '32px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '0.9rem', flexShrink: 0 }}>
                {idx + 1}
              </div>
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                <div style={{ fontWeight: 600, color: 'var(--text-primary)', fontFamily: 'var(--font-display)', fontSize: '1.1rem', lineHeight: 1.4 }}>{normalizeEscapedText(card.front)}</div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                  <select
                    ...
                    style={{ background: 'rgba(0,0,0,0.4)', color: 'var(--text-primary)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 'var(--radius-md)', padding: '0.42rem 0.55rem', fontSize: '0.8rem', fontFamily: 'var(--font-body)' }}
                  >
                  ...
                  <button
                    ...
                    style={{ border: '1px solid rgba(255,255,255,0.18)', background: 'var(--card-bg)', color: 'var(--text-primary)', borderRadius: 'var(--radius-md)', padding: '0.42rem 0.68rem', cursor: rephrasingByCardId[card.id] ? 'not-allowed' : 'pointer', opacity: rephrasingByCardId[card.id] ? 0.65 : 1, display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.78rem', fontFamily: 'var(--font-body)' }}
                  >
